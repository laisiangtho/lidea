...
class KeepPreferences {
  ...
  Future<bool> exists({String? key}) async {
    ...
    return preferences.then((e) => e.containsKey(id(key)));
  }
  ...
  Future<String?> getStr({String? key}) async {
    return preferences.then((e) => e.getString(id(key)));
  }

  Future<bool> setStr({String? key, required String value}) async {
    return preferences.then((e) => e.setString(id(key), value));
  }

  Future<int?> getInt({String? key}) async {
    return preferences.then((e) => e.getInt(id(key)));
  }

  Future<bool> setInt({String? key, required int value}) async {
    return preferences.then((e) => e.setInt(id(key), value));
  }

  Future<bool?> getBool({String? key}) {
    return preferences.then((e) => e.getBool(id(key)));
  }

  Future<bool> setBool({String? key, required bool value}) {
    return preferences.then((e) => e.setBool(id(key), value));
  }

  Future<double?> getDouble({String? key}) {
    return preferences.then((e) => e.getDouble(id(key)));
  }

  Future<bool> setDouble({String? key, required double value}) {
    return preferences.then((e) => e.setDouble(id(key), value));
  }

  /// Returns all keys in the persistent storage.
  Future<Set<String>> get keys async {
    return preferences.then((e) => e.getKeys());
  }
}
